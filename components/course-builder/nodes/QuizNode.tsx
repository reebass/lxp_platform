...
import { ListChecks, Settings, X } from 'lucide-react';
import { dict } from '@/lib/i18n/dictionaries';
import { useEditingContext } from '../CourseBuilderContext';

// Custom React Flow node for Quiz / Assessment blocks.
const QuizNode = memo(({ id, data }: NodeProps) => {
  ...
  return (
    <div className="group relative bg-background border border-violet-400/50 rounded-lg shadow-sm px-2.5 py-1.5 min-w-[140px] max-w-[180px]">
      <Handle
        type="target"
        position={Position.Top}
        className="!bg-violet-500 !w-2.5 !h-2.5 !border-2 !border-background"
      />
      ...
      <div className="flex items-center gap-2">
        <div className="shrink-0 w-6 h-6 rounded-md bg-violet-500/10 flex items-center justify-center">
          <ListChecks size={13} className="text-violet-500" />
        </div>
