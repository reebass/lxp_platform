...
import { PlaySquare, Settings, X } from 'lucide-react';
import { dict } from '@/lib/i18n/dictionaries';
import { useEditingContext } from '../CourseBuilderContext';

// Custom React Flow node for Video content.
const VideoNode = memo(({ id, data }: NodeProps) => {
  ...
  return (
    <div className="group relative bg-background border border-blue-400/50 rounded-lg shadow-sm px-2.5 py-1.5 min-w-[140px] max-w-[180px]">
      <Handle
        type="target"
        position={Position.Top}
        className="!bg-blue-500 !w-2.5 !h-2.5 !border-2 !border-background"
      />
      ...
      <div className="flex items-center gap-2">
        <div className="shrink-0 w-6 h-6 rounded-md bg-blue-500/10 flex items-center justify-center">
          <PlaySquare size={13} className="text-blue-500" />
        </div>
